# Proyecto final: qué debe tener una respuesta completa
...
## Carga de trabajo A: 48 cuadrículas independientes de lado 128, 30 pasos

Se observó en la máquina del autor (números de `data/reference_timings/04_capstone.csv`):
...
- `numba` supera a `numpy` porque una sola función compilada reemplace cuatro
  arrays intermedios por paso; en una cuadrícula de 128x128, esos intermedios dominan.
- `numba_par` es **más lento** que el Numba serial: cada cuadrícula es muy pequeña, así
  que despertar y sincronizar un grupo de hilos 30 veces por cuadrícula cuesta más que el
  trabajo. Los hilos fueron puesto *dentro* del bucle incorrecto.
- El mejor enfoque paralelo para este carga de trabajo es *alrededor* de cuadrículas
  (una tarea por cuadrícula), porque los cuadrículas intercambian cero bytes. En un cluster
  que es un planificador de tareas (Dask, un array de tareas), no es el intercambio de
  mensajes. Un estudiante que añade una candidata que `prange` sobre los 48 cuadrículas
...
## Carga de trabajo B: una cuadrícula de lado 2048, 40 pasos
...
- Un cuadrícula grande y unida es el caso donde los hilos dentro del cuadrícula pagan y donde
...
- En un cluster, este carga de trabajo se mapea a intercambio de mensajes con
  el halo intercambio (dos filas por vecino por paso), no a un planificador de tareas.

## "Una cosa más: el hardware no lo arregla"

Cualquiera de: la carga de trabajo pequeña de cuadrícula (el costo adicional de lanzamiento y
