...
- El tutorial de mpi4py (https://mpi4py.readthedocs.io/en/stable/tutorial.html) cubre las llamadas `Sendrecv` y `reduce` del apartado 3.4. Lea "Point-to-point" y "Collective" y observe la diferencia entre los métodos en minúscula (objetos serializados) y en mayúscula (búferes y arrays de NumPy). El cálculo por vecindad utiliza estos últimos.
...
- En un clúster, el lanzamiento se define mediante un script de trabajo. La documentación de cada sistema determina el lanzador, las opciones y los módulos disponibles.
...
- Empieza con `dask.distributed` en su propio sistema: `pip install "dask[distributed]"`, luego `Client()` sin argumentos inicia trabajadores locales. Las llamadas `client.map` y `client.gather` del apartado 3.5 corren como se escriben. La URL del cuadro de estado que el cliente imprime muestra tareas moviéndose entre trabajadores.
- En un clúster, `dask-jobqueue` (https://jobqueue.dask.org) crea procesos de trabajo mediante tareas por lotes (`SLURMCluster`, `PBSCluster`, ...). Lea "How this works" y "Configuration" antes de consultar la API. La página "Deploy Dask Clusters" enumera otras opciones, como Kubernetes y servicios en la nube.
- Datos más grandes que la RAM: `dask.array` y `dask.dataframe` descomponen el trabajo de NumPy y pandas a través de los trabajadores con la misma API. Pruebe `dask.array.zeros((20000, 20000), chunks=(2000, 2000))` y los cortes del cálculo por vecindad sobre él; el gráfico que construye es el intercambio de halos del Notebook 03, hecho por ti.
