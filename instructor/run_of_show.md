...
El evento se llama exactamente "IEEE Latam Tech".

Todas las personas participantes trabajan en Colab. El instructor comparte un entorno de ejecución de CPU al inicio y uno con GPU a partir del bloque 5. Conviene mantener abiertos `solutions/` y este archivo en otra ventana. Si la sesión se retrasa, se omiten primero la demostración de perfilado (bloque 2, 5 minutos) y luego el experimento con float32 (bloque 6, 5 minutos). Se conservan las comprobaciones de corrección, la explicación de tiempos y transferencias en GPU, el proyecto final y la discusión de cierre. La configuración de GPU termina al cumplir los diez minutos; a las 01:30, quienes no tengan GPU continúan con el modo alternativo.

Grabación del demo de GPU: *(añade enlace después de la puerta 5 en validation.md)*.

## 00:00 Bienvenida y preparación (10 minutos)

- 00:00 Resultados en una diapositiva (los cuatro en el README). "¡Éxito por producir resultados correctos y explicar sus mediciones. No hay un objetivo de aceleración."
- 00:02 Revisión de preparación: todos abren `01_measure_and_multicore` y ejecutan la celda de configuración. Levantan la mano cuando imprime el diccionario de tiempo. Mientras tanto:
  - **Diagnóstico.** La configuración falla por pip: pídeles que vuelvan a ejecutar la celda una vez (red de red transient); aún fallando, los pares con un compañero para este bloque y vuelven a intentar en el descanso. La celda se detiene en "instalando": entorno de ejecución > Reiniciar sesión, volver a ejecutar. Nada corre: entorno de ejecución > Desconectar y borrar el entorno, volver a abrir la enlace. No depurar individuos pasado 00:08.
  - Para quienes no realizaron la preparación, señale el resumen de una página
    (`prep/primer.md`, apartado 1), la celda final de `00_stencil_practice` y la
    versión completa en `solutions/`. No es necesario repetir la explicación de slices para todo el grupo.
- 00:06 Recapitulación del cálculo por vecindad en la diapositiva: borde caliente, promedio de cuatro vecinos, bordes nunca cambian, el bucle de Python pura es la referencia.
...
- 00:14 `%timeit`: señalar las palabras "promedio +- desviación estándar" en la salida. Error común: que `%timeit` informe el mejor tiempo. Informe el promedio de las ejecuciones; `.best` está disponible si se pide.
- 00:17 Predicción, ejecución, explicación: ratio de 128 vs 256. Respuesta: cuatro (tres sumas y el producto); las cortesías asignan nada. Ejecutar la tabla de referencia. Punto de control 2: todos tienen tres filas de NumPy.
...
- 00:42 `prange` y la lista de hilos. Mostrar `MAX_THREADS` en el lienzo compartido: una CPU Colab entorno de ejecución usualmente informa 2. Explicar por qué el cuaderno pregunta al entorno en lugar de asumir.
- 00:46 Predicción, ejecución, explicación: "¿Dos hilos reducirán el tiempo en la mitad?" Resultado típico de Colab T4: entre 1.0x y 1.6x; a veces más lento. Explicaciones para sacar: núcleo físico compartido, velocidad de memoria, arranque de hilos en un pequeño cuadrícula.
...
- 00:55 Gráfico de Amdahl. Ejercicio: elegir la curva más cercana a la medición e
  indicar qué fracción secuencial implica. Aclarar que se trata de un modelo y que
  el ancho de banda de memoria es la causa más probable en este cálculo.
...
Decir a todos que guarden (Ctrl+S). Los estudiantes que quieran probar el GPU pueden hacerlo temprano, pero el bloque comienza a las 13:20 en cualquier caso.

## 01:20 Configuración de GPU (10 minutos, límite de tiempo), celda de configuración del notebook 02

- 01:20 Entorno de ejecución > Cambiar tipo de entorno > GPU. Ejecutar la celda de configuración. Imprime `MODE: GPU` o `MODE: CPU fallback`.
- Diagnóstico: "no GPU disponible" de Colab: modo alternativo, inmediatamente. Error de importación de CuPy en un entorno de GPU: volver a ejecutar una vez; aún fallando, modo alternativo. Reiniciar los bucles: modo alternativo. No gastar el tiempo del entorno en una máquina.
- 01:28 Mostrar el banner de modo alternativo en pantalla para que los estudiantes sepan qué verán: filas de CPU vivas, filas de GPU grabadas con etiqueta de hardware, y las mismas preguntas.
...
- 02:28 Punto de control: una frase por cada carga de trabajo Proyecto final, que fila de la tabla.

## 02:30 Proyecto final (20 minutos), notebook 04
...
- Resultados esperados y razonamiento: `solutions/capstone_solution.md`. La sorpresa enseable es la carga de trabajo A, donde `numba_par` a menudo es más lento que `numba`.
- 02:45 Dos voluntarios leen su recomendación. Preguntar a cada uno: "¿Qué número en su tabla es la base de esta oración?"
- 02:48 Guardar el CSV y el cuaderno.

## 02:50 Revisión (10 minutos)

- Pregunta de salida: "¿Cuándo no ayudaría disponer de más hardware?" Recopilar:
  carga pequeña frente al costo de iniciar hilos, transferencias dentro de un bucle,
  kernel limitado por memoria, fracción secuencial y costo de comunicación.
